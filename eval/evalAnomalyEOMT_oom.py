...
# --- Sottocampionamento per prevenire Out Of Memory (RAM CPU) ---
SUBSAMPLE_FACTOR_IND = 10  # Prende 1 pixel normale ogni 10
SUBSAMPLE_FACTOR_OOD = 1   # Prende tutti i pixel anomali
...
def main():
    ...
    MODEL_CONFIGS = {
        ...
        "coco": {
            "config_path": "../eomt/configs/omt_base_640_coco.yaml",
    ...
    model = build_model(config, img_size, num_classes, coco=is_coco).eval()
        ...
        for dataset_name, ext in DATASETS.items():
            ...
            for img_idx, path in enumerate(image_paths):
                ...
                for method in methods_to_evaluate:
                    ...
                    # Estrazione di tutti i pixel grezzi
                    ood_pixels = anomaly_result[ood_mask].astype(np.float32)
                    ind_pixels = anomaly_result[ind_mask].astype(np.float32)

                    # Sottocampionamento spaziale (slicing) per salvare RAM CPU
                    ood_pixels_sub = ood_pixels[::SUBSAMPLE_FACTOR_OOD]
                    ind_pixels_sub = ind_pixels[::SUBSAMPLE_FACTOR_IND]

                    # Salvataggio nelle liste globali
                    ood_scores_lists[method].append(ood_pixels_sub)
                    ind_scores_lists[method].append(ind_pixels_sub)
                    
                    # Pulizia variabili temporanee del ciclo
                    del ood_pixels, ind_pixels, ood_pixels_sub, ind_pixels_sub, anomaly_result
