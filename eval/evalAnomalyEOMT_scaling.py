        ...
        for dataset_name, ext in DATASETS.items():
            ...
            for img_idx, path in enumerate(image_paths):
                ...
                with torch.no_grad():
                    ...
                    # =========================================================================
                    # AGGREGAZIONE LOGIT COCO -> CATEGORIE CITYSCAPES PER VALUTAZIONE OOD
                    # =========================================================================
                    if args.model in ['coco', 'coco_finetuned']:  # Corretto controllo modelli COCO
                        ...
                        aggregated_scores[0]  = semantic_scores[100]          # road
                        aggregated_scores[1]  = semantic_scores[123]          # sidewalk
                        aggregated_scores[2]  = semantic_scores[[82, 91, 129]].sum(dim=0)  # building
                        aggregated_scores[3]  = semantic_scores[[109, 110, 111, 112, 131]].sum(dim=0) # wall
                        aggregated_scores[4]  = semantic_scores[117]          # fence
                        aggregated_scores[6]  = semantic_scores[9]            # traffic light
                        aggregated_scores[7]  = semantic_scores[11]           # traffic sign
                        aggregated_scores[8]  = semantic_scores[[116, 125]].sum(dim=0) # vegetation
                        aggregated_scores[9]  = semantic_scores[[90, 102, 126]].sum(dim=0) # terrain
                        aggregated_scores[10] = semantic_scores[119]          # sky
                        aggregated_scores[11] = semantic_scores[0]            # person
                        aggregated_scores[13] = semantic_scores[2]            # car
                        aggregated_scores[14] = semantic_scores[7]            # truck
                        aggregated_scores[15] = semantic_scores[5]            # bus
                        aggregated_scores[16] = semantic_scores[6]            # train
                        aggregated_scores[17] = semantic_scores[3]            # motorcycle
                        aggregated_scores[18] = semantic_scores[1]            # bicycle
                        
                        semantic_scores = aggregated_scores
                    # =========================================================================
                    
                    # CORRETTA INDENTAZIONE: Ora il codice è ben allineato sotto il macro-blocco "with"
                    save_dir = f"/content/drive/MyDrive/project/saved_logits/{args.model}/{dataset_name}"
                    os.makedirs(save_dir, exist_ok=True)
                    img_name = os.path.splitext(os.path.basename(path))[0]
                    torch.save(semantic_scores.half().cpu(), os.path.join(save_dir, f"{img_name}.pt"))
