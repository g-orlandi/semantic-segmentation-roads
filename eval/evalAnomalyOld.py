...
def main():
    ...
    # Rimosso l'argomento --method per calcolarli tutti in automatico
    args = parser.parse_args()

    # Lista dei metodi da valutare simultaneamente
    methods_to_evaluate = ['msp', 'maxlogit', 'maxentropy']
    
    # Dizionario per salvare gli score separati per ciascun metodo
    ...
    print(f"Evaluating all methods: {methods_to_evaluate}")
    ...
    for path in glob.glob(os.path.expanduser(str(args.input[0]))):
        ...
        # 1. Carica e prepara prima la Ground Truth (per saltare inferenza inutile)
        ...
        if 1 not in np.unique(ood_gts):
            # Nessuna anomalia, saltiamo il processing di questo frame
            continue
        
        # Aggiungiamo la GT alla lista comune
        ood_gts_list.append(ood_gts)

        # 2. Se ci sono anomalie, eseguiamo l'inferenza del modello
        ...
        # 3. Calcoliamo gli score per TUTTI i metodi contemporaneamente
    ...
    if len(ood_gts_list) == 0:
        print("Nessun dato valido trovato per l'evalutazione.")
        return
    ...
    # Calcolo le metriche per ciascun metodo
    for method in methods_to_evaluate:
