        ...
        for dataset_name, ext in DATASETS.items():
            ...
            for img_idx, path in enumerate(image_paths):
                ...
                with torch.no_grad():
                    ...
                    if class_logits.shape[-1] == 134:
                        coco_to_cityscapes_dict = {
                            0:11,
                            1:18,
                            2:13,
                            3:17,
                        ...
                        mapping_vector = torch.full((133,), 19, dtype=torch.long, device=class_logits.device)
                        for coco_id, city_id in coco_to_cityscapes_dict.items():
                            mapping_vector[coco_id] = city_id
                            
                        # 2. Eseguiamo la rimappatura probabilistica
                        coco_cls_logits = class_logits[..., :133]
                        coco_probs = F.softmax(coco_cls_logits, dim=-1)
                        
                        city_probs = torch.zeros(
                            (*class_logits.shape[:-1], 20), 
                            dtype=class_logits.dtype, 
                            device=class_logits.device
                        )
                        city_probs.index_add_(dim=-1, index=mapping_vector, source=coco_probs)
                        
                        coco_bg_prob = F.softmax(class_logits, dim=-1)[..., 133:134]
                        city_probs[..., 19:20] += coco_bg_prob
                        
                        # Sovrascriviamo class_logits con i logit rimappati a 20 classi
                        eps = 1e-12
                        class_logits = torch.log(city_probs + eps)
